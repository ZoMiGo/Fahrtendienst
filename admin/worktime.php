...
try {
   ...
   $sql = "SELECT end_time,start_time,timediff(end_time,start_time) AS Stunden, datediff(MAX(start_time),MIN(end_time)) AS date FROM auftrege WHERE driver_email = '".$id."' AND start_time>= last_day(now()) + interval 1 day - interval 1 month";
...
$stunden = $results[0]["Stunden"];
        ...
        $pdf->SetXY(10, 168);
        $pdf->Cell(35, 5, "Gesamt Stunden: $hours", 0, 1);
try {
   $sql = "SELECT user_name FROM users WHERE user_email = '".$id."'";
   $stmt = $DB->prepare($sql);
   $stmt->bindValue(":id", intval($_GET["id"]));
...
$pdf->Output("$driver - Stundenliste - $date".".pdf", 'I'); 
?>