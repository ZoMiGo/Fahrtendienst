...
url         = "user_expresauftrege.php?frm_action=3&cid=" +iCustomerId;
...
<?php } ?>
        ...
        <form class="form-horizontal" name="user_expresauftrege" id="user_expresauftrege_form" enctype="multipart/form-data" method="post" action="user_expresauftrege_process_form.php">
            ...
            <textarea class="form-control" rows="3" name="zinfo" id ="zinfo" value="<?php echo $results[0]["zinfo"] ?>"><?php echo $results[0]["zinfo"] ?></textarea>
              ...
              <label class="control-label">Dauer Auftrag</label>
                <select class="form-control select2" name="dauerauftrag" id="dauerauftrag" style="width: 100%;">
                 <option value="0" <?php if($dauer_auftrag=="0") { echo "selected"; } ?> >NO</option>
                 <option value="1" <?php if($dauer_auftrag=="1") { echo "selected"; } ?> >YES</option>
...
<input type="hidden" id="longitude" name="longitude" value="<?php echo $results[0]["longitude"] ?>"/> 
  ...
  </div>Mit der Eingabe Ihrer pers&ouml;nlichen Daten erteilen Sie uns die Erlaubnis, diese zu speichern und f&uuml;r Abwicklungsprozesse innerhalb des Buchungsprozesses zu verwenden. Es werden nur Daten erhoben, gespeichert und genutzt, die unbedingt f&uuml;r die Abwicklungsprozesse notwendig sind.
