...
try {
  ...
  if ($keyword <> "" ) {
    $sql = "SELECT * FROM dispo WHERE driver_id ='$driver_id' AND "
    ...
    //$sql = "SELECT * FROM auftrege WHERE 1 ORDER BY driver_name, timedate, termin"; //nur für Admin 
      $sql = "SELECT * FROM dispo WHERE driver_email ='$driver_email' AND dauer_auftrag ='$auftrag' ORDER BY timedate, termin";
    $stmt = $DB->prepare($sql);
  }
...
<?php } ?>
    ...
    <div class="panel-body">
      ...
      <div class="col-lg-12" style="padding-left: 0; padding-right: 0;" >
        <form action="user_all_dauerauftrege.php" method="get" >
  ...
  for ($i = 1; $i <= $last; $i++) {
    if ($i == $pagenum) {
      ?>
                ...
                ?>
                <li><a href="user_all_dauerauftrege.php?pagenum=<?php echo $i; ?>&keyword=<?php echo $_GET["keyword"]; ?>" class="links"  onclick="displayRecords('<?php echo $page_limit; ?>', '<?php echo $i; ?>');" ><?php echo $i ?></a></li>
                <?php
              }
            }
            ?>
