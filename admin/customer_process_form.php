...
if ($mode == "add_new" ) {
  ...
  if (!$error) {
    $sql = "INSERT INTO `kunden` (`name`, `user_email`, `phone`, `fsw`, `sbehalt`, `kassa`, `SvNr`, `kdnr`, `typ`, `location`, `droplocation`, `mobilitet`, `phoneM`, `phoneF`, `bfahrer`, `zinfo`, `Transportar`, `dauer_auftrag`) VALUES "
         . "( :name, :useremail, :phone, :fsw, :sbehalt, :kassa, :SvNr, :kdnr, :typ, :location, :droplocation, :mobilitet, :phoneM, :phoneF, :bfahrer, :zinfo, :Transportar, :dauerauftrag)";
    try {
      $stmt = $DB->prepare($sql);
  ...
  if (!$error) {
    $sql = "UPDATE `kunden` SET `name` = :name, `user_email` = :useremail, `phone` = :phone, `fsw` = :fsw, `sbehalt` = :sbehalt, `kassa` = :kassa, `SvNr` = :SvNr, `kdnr` = :kdnr, `typ` = :typ, `location` = :location, `droplocation` = :droplocation, `mobilitet` = :mobilitet, `phoneM` = :phoneM, `phoneF` = :phoneF, `bfahrer` = :bfahrer, `zinfo` = :zinfo, `Transportar` = :Transportar, `dauer_auftrag` = :dauerauftrag  "
 . "WHERE user_id = :cid";
