...
if ($mode == "add_new" ) {
  ...
  $dauer_auftrag = "1"; // trim($_POST['dauerauftrag']);
  ...
  $auftrag_typ = "dispo"; //trim($_POST['auftrag_typ']); //"texirequest"; // Vorbereitung für Auftrag Art
...
VALUES ". "(:driverid, :driveremail, :drivername, :senderid, :name, :typ, :sbehalt, :Transportar, :bfahrer, :zinfo, :dauerauftrag, :status :auftrag_typ, :phoneM, :phoneF, :termin, :phone, :droplocation, :location, :latitude, :longitude, :timedate, :disponent, :accept)";
  ...
  header("location:user_dauerauftregenew.php");
  ...
  header("location:user_dauerauftregenew.php?pagenum=".$_POST['pagenum']);
} elseif ( $mode == "delete" ) {
   $cid = intval($_GET['cid']);
   ...
   header("location:user_dauerauftregenew.php?pagenum=".$_GET['pagenum']);
}
