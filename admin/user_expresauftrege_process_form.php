...
if ($mode == "add_new" ) {
  ...
  $dauer_auftrag = "0"; //trim($_POST['dauerauftrag']);
  ...
  $auftrag_typ = "texirequest"; // Vorbereitung für Auftrag Art
  ...
  if (!$error) {
    $sql = "INSERT INTO `texirequest` (`driver_id`, `driver_email`, `driver_name`, `sender_id`, `name`, `typ`, `sbehalt`, `Transportar`, `bfahrer`, `zinfo`, `dauer_auftrag`, `status`, `phoneM`, `phoneF`, `termin`, `phone`, `droplocation`, `location`, `latitude`, `longitude`, `timedate`, `disponent`, `accept`) 
VALUES ". "(:driverid, :driveremail, :drivername, :senderid, :name, :typ, :sbehalt, :Transportar, :bfahrer, :zinfo, :dauerauftrag, :status :auftrag_typ, :phoneM, :phoneF, :termin, :phone, :droplocation, :location, :latitude, :longitude, :timedate, :disponent, :accept)";
  ...
  header("location:user_expresauftrege.php");
  ...
  if (!$error) {
    $sql = "UPDATE `texirequest` SET `driver_id` = :driverid, `driver_email` = :driveremail, `driver_name` = :drivername, `sender_id` = :senderid, `name` = :name, `typ` = :typ, `sbehalt` = :sbehalt, `Transportar` = :Transportar, `bfahrer` = :bfahrer, `zinfo` = :zinfo, `dauer_auftrag` = :dauerauftrag, `status` = :status, `phoneM` = :phoneM, `phoneF` = :phoneF, `termin` = :termin, `phone` = :phone, `droplocation` = :droplocation, `location` = :location, `latitude` = :latitude, `longitude` = :longitude, `accept` = :accept "
    . "WHERE id = :cid ";
  ...
  header("location:user_expresauftrege.php?pagenum=".$_POST['pagenum']);
} elseif ( $mode == "delete" ) {
   $cid = intval($_GET['cid']);
   ...
   header("location:user_expresauftrege.php?pagenum=".$_GET['pagenum']);
}
